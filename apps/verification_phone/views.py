...
class SendSMSView(APIView):
    ...
    def post(self, request):
        ...
        try:
            client = Client(settings.TWILLO_ACCOUNT_SID, settings.TWILIO_AUTH_TOKEN)
            client.verify.v2.services(settings.TWILIO_VEFIFY_SERVICE_SID) \
                .verifications \
                .create(to=phone_number, channel="sms")
        except Exception as e:
            return Response({"message": "Error creating phone number, try again!"},
                            status=status.HTTP_400_BAD_REQUEST)
...
class VerifyPhoneNumberView(APIView):
    ...
    def post(self, request):
        ...
        if not phone_number or not verification_code:
            return Response(
                {"message": "phone_number and verification_code are required"},
                status=status.HTTP_400_BAD_REQUEST)
        try:
